...
execute if score -attacker_jockey crawler_spawns matches 1.. run execute unless score -attacker_jockey_1 crawler_spawns matches 1.. run execute at @e[tag=extinguisher_spawnpoint,limit=1,sort=random] run summon spider ~ ~ ~ {Health:10,NoAI:1b,PersistenceRequired:1b,Tags:["crawlers_crawlerattacker","crawlers_enemy","valid_spell_target","crawlers_crawler_mount"],attributes:[{id:max_health,base:10f},{id:scale,base:1f}],Passengers:[{id:cave_spider,Health:25,NoAI:1b,PersistenceRequired:1b,Tags:["crawlers_crawlerboi","crawlers_enemy","valid_spell_target","crawlers_crawler_rider"],attributes:[{id:scale,base:2f},{id:max_health,base:25f}]}]}

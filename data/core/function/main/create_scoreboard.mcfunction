scoreboard objectives add Team dummy
scoreboard objectives add lobby dummy
scoreboard objectives add settings dummy
scoreboard objectives add Picks dummy
scoreboard objectives add char dummy
scoreboard objectives add LobbyTrigger trigger
scoreboard objectives add memory dummy
scoreboard objectives add score dummy
scoreboard objectives add temp_score dummy

scoreboard objectives add trigger_respawn trigger
scoreboard objectives add trigger_flag_max_countdown trigger
scoreboard objectives add trigger_dmscore trigger
scoreboard objectives add trigger_ctfscore trigger
scoreboard objectives add trigger_acscore trigger

team add yellow
team add purple
scoreboard objectives add KillSpreeCounter playerKillCount
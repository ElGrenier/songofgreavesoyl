...
#title @a[scores={char=20,panic_bite_cd=150..}] actionbar {"text":"Delicious!","color":"green","italic":true,"type":"text"}

execute as @a[scores={char=20,panic_bite_cd=2}] as @e[distance=..5,tag=valid_spell_target] unless score @s Team = @p[scores={char=20}] Team run effect give @s wither 3 5
execute as @a[scores={char=20,panic_bite_cd=2}] as @e[distance=..5,tag=valid_spell_target] unless score @s Team = @p[scores={char=20}] Team run scoreboard players set @s CC_broken 80

effect clear @a[scores={CC_broken=1..}] resistance

scoreboard players add @a[scores={passive_weav=1..,char=20}] panic_bite_cd 1

scoreboard players add @a[scores={char=20,panic_bite_cd=3..,universal_hit=20..}] panic_bite_cd 20
...
execute at @e[tag=snare_webs,scores={entagle=1..5}] as @e[distance=..1.5,tag=valid_spell_target] unless score @s Team = @p[scores={char=20}] Team run tp @s ~ ~ ~
...
item replace entity @a[scores={char=20}] hotbar.0 with minecraft:golden_hoe[minecraft:custom_name={bold:1b,color:"gray",text:"Fang"},minecraft:unbreakable={},minecraft:attribute_modifiers=[{id:"armor",type:"minecraft:attack_damage",amount:3.0d,operation:"add_value",slot:"mainhand"},{id:"armor",type:"minecraft:attack_speed",amount:-0.6d,operation:"add_multiplied_base",slot:"mainhand"}]] 1

...
execute positioned ~ ~-1 ~ if entity @p[distance=..50] unless block ~ ~-1 ~ #minecraft:dash run execute as @e[distance=..4,tag=valid_spell_target] unless score @s Team = @p[scores={char=71}] Team run damage @s 6 generic by @p[scores={char=71}] from @p[scores={char=71}]

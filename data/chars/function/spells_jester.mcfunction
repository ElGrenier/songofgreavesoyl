...
execute at @a[scores={char=51}] as @a unless score @s Team = @p[scores={char=51}] Team run tag @s add jester_dazzle_valid_target
execute at @a[scores={char=51}] as @a if score @s Team = @p[scores={char=51}] Team run tag @s remove jester_dazzle_valid_target
...
execute as @e[tag=dazzle_ball_bounce] at @s run tp @s ~ ~ ~ facing entity @e[sort=nearest,tag=jester_dazzle_valid_target,scores={CC_confusion=0},tag=valid_spell_target,limit=1,distance=0.5..] eyes

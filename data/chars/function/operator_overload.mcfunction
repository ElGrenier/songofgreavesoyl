...
execute positioned ~-.5 ~-.5 ~-.5 as @e[dx=0,dy=0,dz=0,tag=valid_spell_target,tag=!overload] unless score @s Team = @p[scores={char=28}] Team at @a[scores={char=28}] run playsound entity.experience_orb.pickup master @a[scores={char=28}] ~ ~ ~ 0.1 0.1 1
...
execute positioned ^ ^ ^0.3 if entity @s[distance=..20] run function chars:operator_overload